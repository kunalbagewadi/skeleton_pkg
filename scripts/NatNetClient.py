...
def skeletonMessage( id, pos, rot ):
    ...
    currentPose.orientation.x= rot[0]
    currentPose.orientation.y= rot[1]
    currentPose.orientation.z= rot[2]
    currentPose.orientation.w= rot[3]

    #pose_array.poses[id-1]= currentPose
    pose_array.poses.append(currentPose)

    # For upperBody skeleton, ID=13 is last ID. So publish message after ID=13
    if id==13:
        #pub = rospy.Publisher('skeleton1', PoseArray, queue_size=10)
...
class NatNetClient:
    def __init__( self ):
        ...
        self.serverIPAddress = "192.168.1.160"
    ...
    def __processMessage( self, data ):
        ...
        elif( messageID == self.NAT_MODELDEF ):   # messageID==5
